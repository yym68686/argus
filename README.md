...
| `ARGUS_FUGUE_RUNTIME_NETWORK_POLICY` | Optional | `auto` | Controls Fugue `network_policy` on newly created session apps. `required` fails session creation if the gateway app cannot be resolved; `auto` skips the policy when required inputs are missing; `off` disables it. Requires Fugue platform support for App `network_policy`. |
| `ARGUS_FUGUE_RUNTIME_ALLOW_PUBLIC_EGRESS` | Optional | `true` | When runtime network policy is enabled, allow session apps to reach public internet addresses while still blocking private/cluster ranges except explicitly allowed apps such as the gateway. |
...
| `ARGUS_FUGUE_WORKSPACE_STORAGE_CLASS` | Optional | unset | Storage class name for Fugue-managed workspaces, when you need a specific PVC class. The bundled Fugue deployment uses `fugue-workspace-rwo` for movable RWO session workspaces and `fugue-rwx` for shared project workspaces. |
| `ARGUS_FUGUE_WORKSPACE_STORAGE_MODE` | Optional | `dedicated_pvc` | Fugue persistent storage mode for session workspaces. Use `movable_rwo` for Fugue-movable per-session RWO PVCs, or `shared_project_rwx` to mount one per-project RWX PVC with one subdirectory per Argus session. |
...
| `ARGUS_NODE_GATEWAY_URL` | Optional | unset | Gateway base URL used with `ARGUS_NODE_WS_TOKEN` to build `/nodes/ws`. Same as passing `--gateway`. |
| `ARGUS_NODE_HOST` | Optional | unset | Gateway `host[:port]` used with `ARGUS_NODE_WS_TOKEN` to build `/nodes/ws`. Same as passing `--host`. |
| `ARGUS_NODE_WS_TOKEN` | Optional | unset | Node session token used with `ARGUS_NODE_GATEWAY_URL` / `ARGUS_NODE_HOST`. This is not the server-side `ARGUS_NODE_TOKEN` master secret. |
...
- The built-in `gateway` channel is enabled by default in the gateway, but the bundled Docker Compose file disables it on first boot. When enabled, it uses `OPENAI_API_KEY` / `ARGUS_OPENAI_API_KEY` plus `ARGUS_OPENAI_RESPONSES_UPSTREAM_URL`.
