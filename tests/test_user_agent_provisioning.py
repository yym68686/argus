...
class UserAgentProvisioningTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_connection_payload_auto_retries_missing_runtime_image(self) -> None:
        agent_id = "u1-runtime-missing"
            ...
            await self.manager._store.update(
                lambda st: (
                    ...
                    st.sessions.__setitem__(
                        session_id,
                        argus_app.PersistedSessionAutomation(
                            owner_user_id=1,
                            runtime_unrecoverable=True,
                            runtime_error_class="runtime_image_digest_missing",
                            runtime_error_message=failed_agent.provisioning_error,
