...
func (h *Host) connectFrame() connectFrame {
	...
	return connectFrame{
		...
		Caps:        []string{"runtime", "codex", "fs", "mcp"},
	}
}

func (h *Host) Run(ctx context.Context) error {
	h.LogLine("INFO", fmt.Sprintf("Argus runtime-host starting (version=%s)", version.Current()))
	...
	for {
		...
		if err != nil {
			consecutiveFailures++
			h.LogLine("ERROR", fmt.Sprintf("Failed to connect to runtime-host ws (attempt=%d): %s", connectAttempt, err.Error()))
		...
		if everConnected {
			h.LogLine("INFO", "Reconnected to runtime-host ws")
		} else {
			h.LogLine("INFO", "Connected to runtime-host ws")
		}
		everConnected = true
		consecutiveFailures = 0
		...
		go func() {
			...
			for {
				select {
				case <-ticker.C:
					_ = h.sendJSONBestEffort(c, eventFrame{
						Type:  "event",
